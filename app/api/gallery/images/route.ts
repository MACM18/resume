...
import { uploadFile, deleteFile, getPublicUrl, listFiles } from "@/lib/storage";
import { optimizeImage, getImageDimensions } from "@/lib/image-optimization";
import { createGalleryImage, listGalleryImagesForUser, deleteGalleryImageRecord, updateGalleryImage, getUserIdForDomain } from "@/lib/gallery.server";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!userId) {
      return NextResponse.json({ error: "userId or domain is required" }, { status: 400 });
    }
  ...
  try {
    ...
    if (existingFiles.length >= limit) {
      return NextResponse.json(
        { error: `Upload limit reached. You can only upload ${limit} gallery images.` },
        { status: 400 }
      );
    }
    ...
    await uploadFile("gallery-images", filePath, optimizedBuffer, optimizedType);
    const publicUrl = getPublicUrl("gallery-images", `${userId}/${filePath.split("/").pop()}`);
